...
void execArgs(char *args[], int len)
{
    ...
    if (*args[len - 1] == '&')
    { // Nghĩa là tiến trinh f và tiến trình con chạy song song
        flag = 1;
        args[len - 1] = NULL;
        len--;
    }
    ...
    if (child_pid == 0)
    { // Tiến trình đang chạy là tiến trình con
        if (len >= 3)
        { // Nếu là chuyển hướng output
    ...
    { // Không thể  chia được tiến trình
